"""
pipeline.py
Reusable data ingestion + feature engineering functions
"""
...
def get_financials_yahoo(ticker_ns: str) -> Dict[str, Any]:
    try:
        ...
        return {"Ticker": ticker_ns, "MarketCap": 0, "PE_Ratio": 0,
                "DebtToEquity": 0, "ProfitMargins": 0}
    ...
    try:
        ...
        for row in m.values():
            if row.get("ticker","").lower() == ticker_or_adr.lower():
                cik = str(row.get("cik_str")).zfill(10); break
        ...
        recent_forms = [f for f,d in zip(form_list, dates)
                        if d and datetime.datetime.strptime(d, "%Y-%m-%d").date() >= cutoff]
        return {"SEC_used": True, "SEC_recent_180d": len(recent_forms),
...
def get_mca_data(cin: str) -> Dict[str, Any]:
    try:
        ...
        if res.status_code == 200:
            ...
            return {"MCA_used": False, "MCA_error": f"HTTP {res.status_code}"}
    except:
        return {"MCA_used": False, "MCA_error": "Request failed"}
